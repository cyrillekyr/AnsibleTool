        ...
        if grep -q "groupes:" "$groups_file" && grep -q "nom:" "$groups_file" ; then
            ...
            case $target in
                1)
                    ...
                    echo "$servers"
                    # Action
                    ansible-playbook -i "$servers", "$PLAYBOOKS"/user_management/create.yaml --extra-vars "action=adduser utilisateurs_groupes_file=$groups_file"
                ...
                2)
                    ...
                    if [[ $choice -ge 1 && $choice -le $num_groups ]]; then
                        ...
                        ansible-playbook -i "$INVENTORIES"/nodes/"$active_node"/hosts "$PLAYBOOKS"/user_management/create.yaml --extra-vars "action=delgroup utilisateurs_groupes_file=$groups_file"
                        log_action "Deletion of all groups from all servers of $active_node"
                    else
                        echo "Invalid choice"
                    fi
